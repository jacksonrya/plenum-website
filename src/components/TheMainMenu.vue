<template>
  ...
  >
    ...
    >
      ...
      <transition v-if="menu.submenu && menu.submenu.length > 0" name="fly-out-slide">
        <main-menu-fly-out
  ...
  methods: {
    ...
    handleMenuItemHoverEvent: function(event, menu) {
      if (event.type === "mouseenter") {
        ...
        console.error(
          "incorrectly used function: handleMenuItemHoverEvent in TheMainMenu"
        );
      }
    },
    /**
     ...
     */
    openMainMenuFlyOut: function(
      menuItem,
      isKeyboardEvent,
      toLastMenuItem = false
    ) {
     ...
     */
    closeMainMenuFlyOut: function(
      menuItem,
      menuItemIndex,
      returnFocusToMainMenuItem = false
    ) {
    ...
    focusToFlyOut: function(menu, toLastMenuItem = false) {
      ...
      setTimeout(() => {
        document
          .getElementById(menu.title + "-fly-out-menu-item-" + index.toString())
          .focus();
      }, 10);
    },
    /**
      ...
      if (currentlyFocusedIndex !== this.menuItems.length - 1) {
        let el =
          ...
          this.menuItems
            ...
            .findIndex(menuItem =>
              menuItem.title.toLowerCase().startsWith(queryLetter)
            );
...
.main-menu__menu-item {
  ...
  width: calc(#{$lefter-width-desktop} * 2 - 15px * 2);
  height: $menuItemHeight;
  margin: 15px 0;

  line-height: calc(#{$menuItemHeight} + #{$button-text-to-center-adjustment});
...
.main-menu__menu-item-content {
  width: 100%;
  height: calc(#{$menuItemHeight} - #{$button-text-to-center-adjustment});
...
.fly-out {
  ...
  outline: border-desktop;
  // width: calc(100% - calc(#{$lefter-width-desktop} * 2));
  z-index: -1 !important;
}
